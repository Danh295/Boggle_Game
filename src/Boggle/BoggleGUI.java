...
public class BoggleGUI extends JFrame implements ActionListener { // D: thinking about inheriting from BoggleGame class, will have to reimplement GUI frame if so
    ...
    JLabel title, instructions, instructions2, wordLabel, targetScore, countdown;
    static JLabel p1, p2;
    JLabel[][] grid;
    ...
    static boolean doesP1Start = true;
    boolean isTimerRunning;
    public BoggleGUI() {
        setSize(1200, 670); // Setting the size of the frame
        setName("BOGGLE!"); // Naming the window
        setLayout(new GridBagLayout()); // Using a grid bag layout

        // Restart and Exit Buttons
        frameC4 = new GridBagConstraints(); // Constraints for the first component o
        ...
        restart.addActionListener(this);    // Action Listener
        buttons2.add(restart);  // Add button to buttons2 panel
        exit = new JButton("Exit"); // Exit button
        exit.addActionListener(this);   // Action listener on button
        ...
        frameC4.ipady = 30; // How many pixels tall the panel is
        ...
        upper = new JPanel();   // New panel
        upper.setLayout(new GridLayout(3, 1));  // Panel is a grid layout, 1 row for each line
        // Prompts
        ...
        upper.add(instructions);    // Add the prompt
        upper.add(instructions2);   // Add the prompt

        // Timer
        timerCounter = 15;   // Stopwatch starts at 15s, the limit
        countdown = new JLabel("Time remaining for turn: " + timerCounter + "s");    // Label to display time
        //upper.add(countdown);
